...
export default function SingleImageWithText({ data }: Props) {
  const { title, body, cards, cta_label, cta_url } = data;
  const showCta = Boolean(cta_label && cta_url);
  const featuredCard = cards[0] ?? null;

  return (
    <section className="bg-beast-black pb-24 md:pb-32 lg:pb-40">
      <div className="flex min-h-[70vh] flex-col md:flex-row-reverse">
